...
class LayananPengajuanPenyerahanHewanController extends Controller
{
    ...
    public function createPost(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
	        'fotoHewanHandover' => "required",
	        "answers"    => ["required", "array"],  // Ensures "answers" is an array
	        "answers.*"  => ["required"], // Ensures all individual answers are required
	    ]);
		
	    // Check if validation fails
	    if ($validator->fails()) {
	        return redirect()->back()
	            ->withErrors($validator) // Send validation errors to the session
	            ->withInput() // Preserve old input
	            ->with('error', 'Mohon isi semua jawaban dengan benar.');
	    }
